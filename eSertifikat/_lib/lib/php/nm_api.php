...
function sc_call_api($profile, $arr_settings = [])
{
    ...
    switch(strtolower($arr_settings['settings']['gateway']))
    {
        ...
        case 'dropbox':
            ...
            break;
        case 'google_drive':
            include_once($prod_path . "/oauth/google-api-php-client-2.1.3/src/Google/autoload.php");
            $client = new Google_Client();
            $client->setApplicationName(APPLICATION_NAME);
            $client->setScopes(SCOPES);
            $client->setAuthConfig(CLIENT_SECRET_PATH);
            //$client->setRedirectUri(\$this->Ini->server_pdf . \$_SERVER['PHP_SELF']);
    ...
    switch(strtolower($arr_set['gateway'])) {
        case 's3':
            ...
            $result = $instance->putObject([
                ...
                'Key'        => !isset($arr_settings['file_name']) ? basename($arr_settings['file']) : $arr_settings['file_name'],
        ...
        case 'dropbox':
            ...
            $result = $instance->upload($arr_settings['file'], !isset($arr_settings['file_name']) ? basename($arr_settings['file']) : $arr_settings['file_name']);
